...
public class UserInput{
    ...
    public void buttonClick()//按鈕點擊後的動作
    {
        ...
        MainClass mainClass = new MainClass();
        Article result = new Article(text);
        try{
        	 result.segTheSentences();
        	 String location=result.getSeggedSentences().toString();
        	 mainClass.userFootprintViaNLPParser.getJt().setText(location);
        	 mainClass.googleMap.footPrintInMap(result.getSeggedSentencesArray());
        	 mainClass.covid19FootprintDatabase.getJt().setText(mainClass.googleMap.getMatchLocation());
		}
		catch(Exception e){
